...
export default function PendingApproval() {
  ...
  async function handleDevApprove() {
    ...
    try {
      const session = localStorage.getItem("fdf-supabase-session");
      const parsed = session ? JSON.parse(session) : null;
      if (!parsed?.accessToken) throw new Error("No session");
      // Call admin approve via fetch to avoid tRPC hook complexity
      const res = await fetch("/api/trpc/parentApproval.adminApprove", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        credentials: "include",
        body: JSON.stringify({
          json: { accessToken: parsed.accessToken, targetUserId: userId },
        }),
      });
      if (res.ok) {
        await supabase.from("fdf_users").update({ approval_status: "approved" }).eq("auth_user_id", userId);
        navigate("/");
      }
    } catch (e) {
