...
export default function OnboardingUsername() {
  ...
  const saveUsernameMutation = trpc.supabaseAuth.saveUsername.useMutation({
    onSuccess: () => {
  ...
  function handleContinue(e: React.FormEvent) {
    ...
    saveUsernameMutation.mutate({ accessToken, username: trimmed });
  }
    ...
    <div style={S.page}>
      <div style={S.card}>
        ...
        <form onSubmit={handleContinue} style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          ...
          <button
            type="submit"
            disabled={saveUsernameMutation.isPending || !isValid}
            style={{ ...S.submitBtn, opacity: saveUsernameMutation.isPending || !isValid ? 0.6 : 1, cursor: saveUsernameMutation.isPending || !isValid ? "not-allowed" : "pointer" }}
          >
